                        ...
                        if d[i + 4] & ((0x01 << j) & 0xFF):
                            if (d[2] & 0x40):
                                if (i * 8 + j) == 0:
                                    ...
                                    h12 = (str(i * 8 + j + 1) + 'AM') if ((i * 8 + j) < 12) else (
                                                str(i * 8 + j - 11) + 'PM')
                                h = (h + ';') if n > 0 else h
                                h = h + h12
                            else:
...
class PiJuiceConfig(object):
    ...
    batteryProfiles = ['PJZERO_1000', 'BP7X_1820', 'SNN5843_2300', 'PJLIPO_12000', 'PJLIPO_5000', 'PJBP7X_1600',
                       'PJSNN5843_1300', 'PJZERO_1200', 'BP6X_1400', 'PJLIPO_600', 'PJLIPO_500', 'PJLIPO_2500']

    def SelectBatteryProfiles(self, fwver):
        if fwver >= 0x15:
            self.batteryProfiles = self.batteryProfiles
        elif fwver >= 0x14:
            self.batteryProfiles = ['PJZERO_1000', 'BP7X_1820', 'SNN5843_2300', 'PJLIPO_12000', 'PJLIPO_5000',
                                    'PJBP7X_1600', 'PJSNN5843_1300', 'PJZERO_1200', 'BP6X_1400', 'PJLIPO_600',
                                    'PJLIPO_500']
        elif fwver == 0x13:
            self.batteryProfiles = ['BP6X_1400', 'BP7X_1820', 'SNN5843_2300', 'PJLIPO_12000', 'PJLIPO_5000',
    ...
    def GetBatteryProfile(self):
        ...
        if ret['error'] != 'NO_ERROR':
            ...
            profile['capacity'] = 0xFFFFFFFF if (packed_u16 == 0xFFFF) else (packed_u16 & 0x7FFF) << (
                        ((packed_u16 & 0x8000) >> 15) * 7)
        ...
        try:
            ...
            if (cap == 0xFFFFFFFF):
                ...
                packed_u16 = (c >> (int(c >= 0x8000) * 7)) | int(
                    c >= 0x8000) * 0x8000  # correction for large capacities over 32767
            d[0] = packed_u16 & 0xFF
    ...
    def GetIoConfiguration(self, io_pin):
        ...
        if ret['error'] != 'NO_ERROR':
            ...
            pull = self.ioPullOptions[(d[0] >> 4) & 0x03] if (
                        ((d[0] >> 4) & 0x03) < len(self.ioPullOptions)) else 'UNKNOWN'
            if mode == 'DIGITAL_OUT_PUSHPULL' or mode == 'DIGITAL_IO_OPEN_DRAIN':
                ...
                wup = self.ioConfigParams['DIGITAL_IN'][0]['options'][d[1] & 0x03] if d[1] & 0x03 < len(
                    self.ioConfigParams['DIGITAL_IN'][0]['options']) else ''
